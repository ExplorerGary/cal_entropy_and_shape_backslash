...
def cal_percentile(pt_path:str,fp64_enable:bool = False, pure_data_enable:bool = False, scaling = int(1e6)) -> float:
    """
    计算熵
    ...
    """
    _90th_percentile = None
    _95th_percentile = None
    _100th_percentile = None 
    try:
        name = os.path.basename(pt_path)
        pt_array = read_pt(pt_path).astype(np.float64)
        quantized = np.abs(pt_array) if pure_data_enable else np.abs(quantlization(pt_array=pt_array,
                                                                    scaling = int(1e6), 
                                                                    fp64_enable = fp64_enable,
                                                                    debug = True))
        # 按照(量化)后的每一个数据值进行计算

        np.percentile(quantized, 90)
        np.percentile(quantized, 95)
        np.percentile(quantized, 100)
        
        return {
            "name":name, 
            "90th percentile":_90th_percentile,
            "95th percentile":_95th_percentile,
            "100th percentile":_100th_percentile,
            }
...
def local_test():
    ...
    for pt_path in avail_pt:
        start = time.time()
        result = cal_percentile(pt_path,fp64_enable=True,pure_data_enable=True,scaling=int(1e8))
        end = time.time()
